...
export default function SpecialtiesSearch() {
  ...
  const filteredSpecialties = specialtiesData.filter(sp => {
  ...
  return (
    <div className="w-full bg-[#F8FAFC] py-32 px-4 sm:px-6 lg:px-8 border-t border-gray-100">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 auto-rows-fr">
          {displayItems.map((sp, i) => (
            <motion.div
              ...
              onClick={() => {
                 // Assuming navigate is imported and used in SpecialtiesSearch
                 window.location.href = `/specialty/${encodeURIComponent(sp.name)}`;
              }}
            >
        ...
        {!showAll && filteredSpecialties.length > 9 && (
